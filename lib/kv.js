...
// Atomik hisoblagich: birinchi marta yaratilganda muddat (TTL) qo'yiladi.
// Kunlik AI-so'rov limitlari uchun ishlatiladi. Yangi qiymatni qaytaradi.
export async function bumpCounter(name, ttlSeconds) {
  const c = await client();
  const n = await c.incr(name);
  if (n === 1 && ttlSeconds) await c.expire(name, ttlSeconds);
  return n;
}
