...
const IAS16 = {
  ...
  examples:[
    ...
    { title:"ЗАДАЧА. Себестоимость построенного завода (IAS 16 + IAS 23)",
      problem:"В 2017 г. компания строила завод; он готов к использованию 31 августа 2017 (ввод в эксплуатацию — начало 2018). Затраты (£'000): земля 10 000; подготовка площадки 1 000; оплата труда строителей (1 мар–31 авг) 2 000; стройматериалы 2 200 (из них 700 списаны из-за порчи при пожаре); доля общих управленческих расходов 2 420; проценты 880 (11 мес. по 8% годовых с 12 млн заёмных, привлечённых частично для стройки). Рассчитайте максимальную себестоимость завода. Верно ли, что в 2017 г. износ не начисляется?",
      steps:[["Земля", "10 000 — включается."], ["Подготовка площадки", "1 000 — прямые затраты, включается."], ["Оплата труда строителей", "2 000 — прямые затраты, включается."], ["Стройматериалы", "2 200 − 700 (сверхнормативные потери от пожара — расход) = 1 500."], ["Общие управленческие расходы", "2 420 — НЕ включаются (не относятся напрямую к доведению актива)."], ["Проценты (IAS 23)", "Капитализируют только за период стройки (1 мар–31 авг = 6 мес.): 8%×12 000×6/12 = 480. Остальные 400 — расход."], ["Итого себестоимость", "10 000 + 1 000 + 2 000 + 1 500 + 480 = 14 980."]],
      answer:"Себестоимость завода = 14 980 (£'000). Предположение НЕВЕРНО: амортизация начинается с готовности к использованию (31 августа 2017), а не с ввода в эксплуатацию, — значит в 2017 г. износ за ~4 месяца начисляется." },
  ],
  quiz:[
